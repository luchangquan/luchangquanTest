 ...
 * @version 1.0
 */
public class AppAudioNoticeSignConsumerService extends NoticeSignConsumerService implements IAppAudioNoticeSignConsumerService {
	private static final Logger _LOGGER = LoggerFactory.getLogger(AppAudioNoticeSignConsumerService.class);
	...
	public void saveMessage(String message) throws InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException, NoSuchProviderException,
		...
		String userAccount = appAudioNoticeRequestData.getUserAccount();
		...
		String signKey = appCode + "/" + userAccount + "/" + utcTimeStr + "/" + appAudioNoticeRequestData.getNoticeIdentities().get(0).getFileIdentity() + "_sign.xml";
		...
		// 保存证据
		super.saveEvidence(CategoryEnum4MEvidence.APPVOICE, noticeRequestSignatureVo, aliOssBucketEnum, signKey, evidencesId, signatureKey);
		// 对应关系加入缓存
