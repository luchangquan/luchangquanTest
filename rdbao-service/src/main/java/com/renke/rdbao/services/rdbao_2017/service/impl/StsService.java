...
public class StsService implements IStsService {
	...
	public AliOssStsVo getUploadOssSts(RequestSignData requestSignData, UserContext userContext) throws RdbaoException, UserContextException, AliOperateException {
		...
		AliOssStsVo aliOssStsVo = AliOssUtil.getUploadFileSts(AliOssBucketEnum.getAliOssBucketEnumByName(bucketName), FileTypeEnum.getFileTypeEnumByCode(fileType), Lists.newArrayList(fileIdentity));
		if (appCode.equalsIgnoreCase("AppVideo")) {
			aliOssStsVo.setCallbackUrl(PropertiesConfUtil.PROPERTIES_CONF.getAliOssUploadCallbackConf().getAppVideoCallbackUrl());
		} else if (appCode.equalsIgnoreCase("AppVoice")) {
			aliOssStsVo.setCallbackUrl(PropertiesConfUtil.PROPERTIES_CONF.getAliOssUploadCallbackConf().getAppAudioCallbackUrl());
		} else if (appCode.equalsIgnoreCase("AppPicture")) {
			aliOssStsVo.setCallbackUrl(PropertiesConfUtil.PROPERTIES_CONF.getAliOssUploadCallbackConf().getAppPictureCallbackUrl());
		}
		aliOssStsVo.setEndpoint(aliOssStsVo.getEndpoint().replace("-internal", ""));
		return aliOssStsVo;
	}
	...
	public AliOssStsVo getUploadOssSts4Voice(RequestSignData requestSignData) throws AliOperateException, RdbaoException {
		...
		this.checkUploadOssSts4Voice(appCode, userAccount, callTime, CallTypeEnum4MEvidenceTelecomVoice.getCallTypeEnumByCode(callType), fileIdentity);
	...
	private void checkUploadOssSts4Voice(String appCode, String userAccount, String callTime, CallTypeEnum4MEvidenceTelecomVoice callType, String fileIdentity) throws RdbaoException {
