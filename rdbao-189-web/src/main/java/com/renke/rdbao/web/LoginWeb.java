...
public class LoginWeb extends BaseWeb {
	...
	@Autowired
	private ILogin189Service login189Service;
	@Autowired
	private ILogout189Service logout189Service;
	@Autowired
	...
	public Map login(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (type != null && type.trim() != "") {
				if (type.equals("1")) {
					userContext = login189Service.login(account, passWord, UserTypeEnum.MANAGER);
				} else if (type.equals("2")) {
					userContext = login189Service.login(account, passWord, UserTypeEnum.PERSONAL);
				} else {
					userContext = login189Service.login(account, passWord, UserTypeEnum.PERSONAL);
				}
			} else {
			...
			if (weakPassword) {// 弱密码强制退出
				logout189Service.logout(userContext.getAccessToken());
			} else {
				// 写入 cookie
	...
	public String logout(HttpServletRequest request, HttpServletResponse response) {
		// 读取 accessToken
		try {
			logout189Service.logout(getAccessTokenFromCookie());
		} catch (UserContextException e) {
	...
	ResponseData getUserInfo(String account, Short userType, String opType, HttpServletRequest request, HttpServletResponse response) {
		...
		EnhancedEUser189 enhancedUser189 = new EnhancedEUser189();
		enhancedUser189 = eUser189Service.getEnhancedByAccount(account, type, null);
	...
	ResponseData sendVerificationCode(String account, String smsTemplateCode, Short userType, HttpServletRequest request, HttpServletResponse response) {
		...
		EnhancedEUser189 enhancedUser189 = eUser189Service.getEnhancedByAccount(account, type, null);
		try {
	...
	ResponseData updatePassword(String account, String password, Short userType, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			eUser189Service.updatePassword4User(phoneOrEmail, type, password, null);
