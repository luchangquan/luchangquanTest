...
public class BaseController {
	...
	@Autowired
	private IDSourceService sourceService;
	...
	public void checkApiSign(RequestData requestData) {
		...
		EnhancedDSource source = sourceService.getEnhancedByCode(sourceCode, null);
		if (source == null) {
			throw new RdbaoException("[来源未知]");
		}
		if (source.getStatus() != StatusEnum4DSource.NOT_CLOSE) {
			throw new RdbaoException("[来源未启用]");
		}
		String sign = DigestUtils.md5Hex(sourceCode + "_" + requestTime + "_" + source.getKey());
