 ...
 */
@Component
public class AppPictureNoticeSignCustomer {
	private static final Logger _LOGGER = LoggerFactory.getLogger(AppPictureNoticeSignCustomer.class);
	...
	private static IAppPictureNoticeSignConsumerService appPictureNoticeSignConsumerService;
	// 消费的队列
	private static final String QUEUE_NAME = AliMnsQueueTemplateConstants.RDBAO_EVIDENCE_APP_PICTURE_NOTICE_SIGN;

	@Autowired
	public void setAppPictureNoticeSignService(IAppPictureNoticeSignConsumerService appPictureNoticeSignConsumerService) {
		AppPictureNoticeSignCustomer.appPictureNoticeSignConsumerService = appPictureNoticeSignConsumerService;
	}
	...
	public static void start() {
		...
		for (int i = 0; i < Integer.valueOf(PropertiesConfUtil.PROPERTIES_CONF.getMnsConsumerConf().getAppPictureNoticeSignCustomerCount()); i++) {
	...
	private static void checkScheduleStatus() {
		...
		timer.schedule(new TimerTask() {
			public void run() {
				_LOGGER.info("[APP图片签名消费者-开始检测任务运行情况....]");
				...
				while (scheduleIterator.hasNext()) {
					...
					if (schedule.getState() == State.TERMINATED) {
						_LOGGER.warn("[APP图片签名消费者-任务停止,重启任务:{}]", taskId);
						start(taskId);
						_LOGGER.warn("[APP图片签名消费者-任务停止,重启成功:{}]", taskId);
					} else {
						_LOGGER.info("[APP图片签名消费者-任务正常运行:{},{}]", taskId, schedule.getState());
					}
				}
				_LOGGER.info("[APP图片签名消费者-检测任务运行情况完毕]");
			}
		}, 3 * 60 * 1000, 3 * 60 * 1000);
	}
	...
	private static class Schedule implements Runnable {
		...
		public void run() {
			...
			index = ++scheduleCount;
			_LOGGER.info("[APP图片签名消费者启动:" + Thread.currentThread().getName() + "   ,index=" + index + "]");
			while (true) {

				try {
					doSchedule();
				} catch (Exception ex) {
					_LOGGER.error("[APP图片签名消费者未知异常:" + Thread.currentThread().getName() + "]", ex);
				}
		...
		private void doSchedule() {
			...
			try {
				...
				_LOGGER.info("[APP图片签名数据:{}", messageStr);
				appPictureNoticeSignConsumerService.saveMessage(messageStr);
				...
				if (ex.getMessage().trim().equals("Connection closed") || ex.getMessage().trim().startsWith("Connection reset")) {
					...
					_LOGGER.debug("[APP图片签名消费者MNS连接异常,可忽略：{}，{}，{}]", ex.getErrorCode(), ex.getRequestId(), ex.getMessage());
				}
			} catch (Exception ex) {
				if (ex instanceof BaseException) {
					BaseException baseException = (BaseException) ex;
					if (baseException.getResponse() == ResponseEnum.FILE_SUMMARY_INCONSISTENCY) {
						_LOGGER.info("[APP图片签名消费者--文件摘要不匹配，删除通知:{}]", message.getMessageBodyAsRawString());
						queue.deleteMessage(message.getReceiptHandle());
					}
				} else {
					_LOGGER.error("[APP图片签名消费者未知异常:" + Thread.currentThread().getName() + "]", ex);
