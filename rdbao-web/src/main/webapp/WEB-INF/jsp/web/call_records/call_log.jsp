...
<div class="callDown">
    <div class="downCon">
        <p>语音下载  请在30分钟内提交下载申请，超过30分钟列表数据将失效。<span class="downConClose" title="关闭"></span></p>
        <div class="downList">
            <div class="down01">
                <table>
                    <thead >
...
<section class="main">
    ...
    <div class="date-tit font12">
        ...
        <input value="" type="hidden" id="startTimeStr"/>
        <input value="" type="hidden" id="endTimeStr"/>
        <input value="" type="hidden" id="daysNumber"/>
    ...
    <div class="callLog-con-box">
        ...
        <div class="call-content-con">
            <table class="call-content" id="call-content">
            </table>
        </div>
        <!--END 通话记录列表-->
        <!--按钮-->
...
<script>
	...
	function forAjax(data){
		...
		var url="${baseUrl}/callRecords/getBlackAndWhiteListData";
		var ajaxJson = (ajaxCustom(url,(data), 'POST', false));
		var html="";
		console.log(ajaxJson);
		
		//$("#startTimeStr").val("2016-09-26");
		$("#startTimeStr").val(ajaxJson.startTimeStr);
		$("#endTimeStr").val(ajaxJson.endTimeStr);
		$("#daysNumber").val(ajaxJson.daysNumber);
		$("#totalPages").val(ajaxJson.totalPages);
		$("#index").val(ajaxJson.index);
		
		
		var objectAaary=ajaxJson.listCallLogData;
		for(row in objectAaary){
		
			html+="<tr>";
			html+="<td class='checkboz'><label><input value='"+objectAaary[row]['id']+"' type='checkbox' name='call-ba' id='all'><i></i></label></td>";
			html+="<td class='call-play'><span title='在线播放'><input value='"+objectAaary[row]['fileUrl']+"' type='hidden' /></span></td>";
			html+="<td class='call-type'>"+objectAaary[row]['callType']+"</td>";
			html+="<td class='hj-no'>"+objectAaary[row]['callOutPhone']+"</td>";
			html+="<td class='hc-no'>"+objectAaary[row]['callIncomingPhone']+"</td>";
			html+="<td class='ly-ks'>"+objectAaary[row]['recordIngTimeStr']+"</td>";
			html+="<td class='ly-js'>"+objectAaary[row]['recordEndTimeStr']+"</td>";
			html+="<td class='th-time'>"+objectAaary[row]['conversationTime'] +"</td>";
			html+="<td class='th-notary'><a href='"+objectAaary[row]['evidRecordViewUrl']+"' target='_blank'>"+objectAaary[row]['notaryName']+"</a></td>";
			html+="</tr>";
		}
	...
	function onDays(day){		
		    var searchPhoneNo=$("#searchPhoneNo").val();
 		    ...
 		    var data={'searchPhoneNo':searchPhoneNo,"daysNumber":day};
   	    	forAjax(data);
	}
...
//             audiosrc = 'http://localhost:8080/rdbao-web/file/donwload'; //测试
//             audiosrc='http://7xwe1x.com1.z0.glb.clouddn.com/audio/20160713/henanzhuizi-daminqiyuan1.mp3';
 		...
 		$(".call-down-bt").click(function(){
 	        ...
 	        var emailY='<p><i>邮件验证码：</i><input type="text" value="" name="emailNumber" placeholder="输入邮件验证码"><span  id="time" class="">发送验证码</span></p>';
 	        	...
 	        	var url="${baseUrl}/callRecords/saveDownloadInfo";
	 	   		$.ajax({
	 				...
	 				success : function(responseJson) {
							...
							var downloadEvidencesVo=responseJson.downloadEvidencesVo;
							//如果下载的证据大于1条记录，通过发送邮件的方式下载 否则直接下载
	 						if(downloadEvidencesVo.count>1){
	 							...
	 							if(userEmail!=null&&$.trim(userEmail)!=""){
	 								$(".downNumber").html(downloadEvidencesVo.count);//写入数量
		 							$(".downBox").show();
	 							}else{
            	...
            	$("input[name=call-ba]:checked").each(function(){
            			...
            			if($("#tbody").find("tr").length>0){
                			...
                			$("#tbody").find("tr").each(function(){
                				var tableId=$(this).find("td").eq(0).find("input").val();
                				if(id==tableId){
                					count+=1;
                				}
        ...
        $(document).on("click", ".search-bt", function () {
            ...
            if (searchPhoneNo != "") {
                ...
                var data={'searchPhoneNo':searchPhoneNo,'startTime':start,'end':end};
                forAjax(data);
            } else if (start != "" || end != "") {
            	  swal({title: "", text: "正在查询，请稍后"});
            	  var data={'searchPhoneNo':searchPhoneNo,'startTime':start,'end':end};
...
//                 					 window.location="${baseUrl}/callRecords/callLog";//刷新当前页面.
                ...
                setPage(container, count, inx);
	   	 		...
	   	 		var end=$("#end").val();
	   	 		 ...
	   	 		 if(start==""&&end==""){
	            	  ...
	            	  var data={'searchPhoneNo':searchPhoneNo,'daysNumber':day,'index':inx};
	            	  forAjax(data);

	   	 		 }else{
	            	  var data={'searchPhoneNo':searchPhoneNo,'startTime':start,'endTime':end,'index':inx};
	            	  forAjax(data);
    ...
    function counttime(time) {
        ...
        if (time > 0) {
            ...
            window.setTimeout("counttime(" + time + ")", 1000);
