...
public class VoiceStatisticsService implements IVoiceStatisticsService {
	@Autowired
	private IMEvidenceDao mEvidenceDao;
	@Autowired
	...
	public List<VoiceStatisticsVo> getVoiceStatisticsShowMonths(UserContext userContext) throws UserContextException {
		...
		// }
		// EUser189 firstEUser189 =
		// this.getFirstCreateEUser189(userContext.getContainEUser189s());
		//
		// // 用户创建的年月等于当前的年月
		// DateTime firstUserCreateTime = new
		// DateTime(firstEUser189.getCreateTime());
		...
		// Date startDate =
		// firstEUser189.getCreateTime().before(dateTime.toDate()) ?
		// dateTime.toDate() : firstEUser189.getCreateTime();
	...
	private EUser189 getFirstCreateEUser189(List<EUser189> containEUser189s) {
		EUser189 firstEUser189 = containEUser189s.get(0);
		if (containEUser189s.size() == 1) {
			return firstEUser189;
		}
		for (EUser189 _EUser189 : containEUser189s) {
			if (_EUser189.getCreateTime().before(firstEUser189.getCreateTime())) {
				firstEUser189 = _EUser189;
			}
		}
		return firstEUser189;
	}
	...
	public VoiceStatisticsVo getTotalVoiceStatistics(Date startTime, Date endTime, UserContext userContext) {
		...
		List<TotalVoiceStatisticsVo> totalVoiceStatisticsVos = mEvidenceDao.getTotalVoiceStatistics(startTime, endTime, userContext.getContainUserIds());
		...
		for (TotalVoiceStatisticsVo _TotalVoiceStatisticsVo : totalVoiceStatisticsVos) {
			if (_TotalVoiceStatisticsVo.getCallType() == CallTypeEnum4MEvidenceTelecomVoice.CALLING.getCode()) {
			...
			} else if (_TotalVoiceStatisticsVo.getCallType() == CallTypeEnum4MEvidenceTelecomVoice.CALLED.getCode()) {
	...
	public VoiceTotalTimeStatisticsVo getVoiceTotalTimeStatistics(Date startTime, Date endTime, String searchPhoneNoOrNickname, UserContext userContext) {
		...
		List<Map<String, Object>> reportMaps = mEvidenceDao.getVoiceTotalTimeStatistics(startTime, endTime, searchUserIds);
	...
	private List<String> getListByLikePhoneNoOrNickname4Company(String searchPhoneNoOrNickname, String companyId, List<UserTypeEnum> types, ArrayList<StatusEnum4User> statuses,
		...
		if (userContext.getUserTable() == UserTableEnum.E_189_USER) {
			// TODO 暂时按照普通用户与管理员分
			List<EUser189> EUser189s = eUser189Dao.getListByLikePhoneNoOrNickname4Company(searchPhoneNoOrNickname, companyId, types, statuses);
	...
	@Override
	public VoiceCycleDateStatisticsVo getVoiceCycleDateStatistics(Date startTime, Date endTime, String searchPhoneNoOrNickname, List<CallTypeEnum4MEvidenceTelecomVoice> callTypes,
			Pagination pagination, UserContext userContext) throws UserContextException {
		...
		List<Map<String, Object>> reports = mEvidenceDao.getVoiceCycleDateStatistics(startTime, endTime, callTypes, shownames, searchUserIds, pagination);
	...
	private List<String> getShownames(List<String> searchUserIds, UserContext userContext) {
		List<String> shownames = new ArrayList<String>();
		if (userContext.getUserTable() == UserTableEnum.E_189_USER) {
			List<EUser189> searchEUser189s = eUser189Dao.getListByKeyValues(EUser189.FIELD_ID, new ArrayList<Object>(searchUserIds), EUser189.class);
			for (EUser189 _EUser189 : searchEUser189s) {
				String showname = Detect.notEmpty(_EUser189.getNickname()) ? _EUser189.getNickname() : _EUser189.getPhoneNo();
	...
	public TotalVoiceCycleDateQuantityStatisticsVo getTotalVoiceCycleDateQuantityStatisticsVo(Date startTime, Date endTime, String searchPhoneNoOrNickname,
			List<CallTypeEnum4MEvidenceTelecomVoice> callTypes, UserContext userContext) {
		...
		totalVoiceCycleDateQuantityStatisticsVo.setTotal(mEvidenceDao.getTotalVoiceCycleDateQuantityStatisticsVo(startTime, endTime, callTypes, searchUserIds));
	...
	@Override
	public VoiceSpecifiedDateStatisticsVo getVoiceSpecifiedDateStatistics(Date startTime, Date endTime, String searchPhoneNoOrNickname, List<CallTypeEnum4MEvidenceTelecomVoice> callTypes,
			UserContext userContext) {
		...
		List<Map<String, Object>> reportMaps = mEvidenceDao.getVoiceSpecifiedDateStatistics(startTime, endTime, callTypes, shownames, searchUserIds);
		List<String> titles = new ArrayList<String>();
		titles.add("时间段");
		titles.addAll(shownames);
		List<Map<String, Object>> finalReprot = new ArrayList<Map<String, Object>>();
	...
	@Override
	public TotalVoiceSpecifiedDateQuantityStatisticsVo getTotalVoiceSpecifiedDateQuantityStatistics(Date curTime, String searchPhoneNoOrNickname, List<CallTypeEnum4MEvidenceTelecomVoice> callTypes,
			UserContext userContext) {
		...
		totalVoiceSpecifiedDateQuantityStatisticsVo.setTotal(mEvidenceDao.getTotalVoiceSpecifiedDateQuantityStatistics(curTime, callTypes, searchUserIds));
