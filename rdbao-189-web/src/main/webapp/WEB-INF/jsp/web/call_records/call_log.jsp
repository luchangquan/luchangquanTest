...
<div id="NotaryBox">
    <div class="NotaryCon">
        ...
        <div class="NotaryLi">
            ...
            <div id="Notary03">
                <div class="oryysx">
                    <label><i>委托人姓名：</i><input id="agentName" type="text" value=""/><b>* 委托书姓名</b></label>
                    <label><i>公证主体　：</i><input id="notarySubject" type="text" value=""/><b>* 公证主体</b></label>
                    ...
                    <label><i>事项名称　：</i><input id="matterName" type="text" value="" maxlength="30"/><b>* 公证事项标题</b></label>
					...
					<c:when test="${not empty listCallLogData}">
						 <c:forEach items="${listCallLogData}" var="item">
						 		<tr>
				                    ...
				                    <td class="call-play"><span title="在线播放"><input value="${item.id}" type="hidden" /></span></td>
				                    ...
				                    <td class="th-notary">${item.notaryName}</a></td>
               				  </tr>
						 </c:forEach>
					</c:when>
			</c:choose>
...
function ajaxCustom(url, data, method, async) {
    ...
    $(document).ready(function(){
        ...
        $(document).on("click",".call-play span",function(){
            var id=$(this).find("input").val();//文件地址
        	var url="${baseUrl}/callRecords/getEnhanced";
        	var audiosrc="";
    		$.ajax({
    			url : url,
    			data :{"id":id} ,
    			...
    			success : function(responseText) {
    				 if(responseText.success!=true){
    					 swal("获取播放地址失败!",responseText.success, "error");
    					 
    				 }else{
    					 audiosrc=responseText.callLogData.fileUrl;
    				 }
        ...
        $(NoNext).on("click",function(){
            ...
            if(NotaryLiDiv==2){
                    	...
                    	var dataJson={"id":arr,"notarySubject":$("#notarySubject").val(),"phoneNo":$("#phoneNo").val(),"email":$("#email").val(),"matterName":$("#matterName").val(),"matterDesc":$("#matterDesc").val(),"agentName":$("#agentName").val()};
...
//--------------批量下载按钮 end---------------
        ...
        $(".downConClose").on("click",function(){
 		...
 		$(".call-down-bt").click(function(){
			...
			var downNumber=$(".downNumber").text();
 	        if(parseInt(downNumber)>=30){
	        	 swal({title:"",text:"每次最多下载30条数据！"});
	        	 return false;	
	        }
 ...
 //查找是否申请公证过，如果是取最近一条
 function getLatelyApplyNotary() {
	 var dataJson={};
 	var url="${baseUrl}/callRecords/getLatelyApplyNotary";
		$.ajax({
			url : url,
			data :dataJson ,
			type : "POST",
			...
			success : function(responseText) {
				console.log(responseText);
					 var enhancedENotarizationReserve = responseText['enhancedENotarizationReserve'];
					 console.log(enhancedENotarizationReserve);
 					 $("#agentName").val(enhancedENotarizationReserve.agentName);
 						$("#notarySubject").val(enhancedENotarizationReserve.notarySubject);
 						$("#phoneNo").val(enhancedENotarizationReserve.phoneNo);
 						$("#email").val(enhancedENotarizationReserve.email);
},
			error : function(msg) {
				alert(msg);
			}
