 ...
 */
@Component
public class RdpNoticeSignCustomer {
	private static final Logger _LOGGER = LoggerFactory.getLogger(RdpNoticeSignCustomer.class);
	...
	private static IRdpNoticeSignConsumerService rdpNoticeSignConsumerService;
	// 消费的队列
	private static final String QUEUE_NAME = "rdbao-evidence-rdp-notice-sign";
	// 配置启动任务个数
	private static final int SCHEDULE = Integer.valueOf(PropertiesConfUtil.PROPERTIES_CONF.getMnsConsumerConf().getRdpNoticeSignCustomerCount());

	@Autowired
	public void setAppVideoNoticeSignService(IRdpNoticeSignConsumerService rdpNoticeSignConsumerService) {
		RdpNoticeSignCustomer.rdpNoticeSignConsumerService = rdpNoticeSignConsumerService;
	}
	...
	private static void checkScheduleStatus() {
		...
		timer.schedule(new TimerTask() {
			public void run() {
				_LOGGER.info("[RDP签名消费者-开始检测任务运行情况....]");
				...
				while (scheduleIterator.hasNext()) {
					...
					if (schedule.getState() == State.TERMINATED) {
						_LOGGER.warn("[RDP签名消费者-任务停止,重启任务:{}]", taskId);
						start(taskId);
						_LOGGER.warn("[RDP签名消费者-任务停止,重启成功:{}]", taskId);
					} else {
						_LOGGER.info("[RDP签名消费者-任务正常运行:{},{}]", taskId, schedule.getState());
					}
				}
				_LOGGER.info("[RDP签名消费者-检测任务运行情况完毕]");
			}
		}, 3 * 60 * 1000, 3 * 60 * 1000);
	}
	...
	private static class Schedule implements Runnable {
		...
		public void run() {
			...
			index = ++scheduleCount;
			_LOGGER.info("[RDP签名消费者启动:" + Thread.currentThread().getName() + "   ,index=" + index + "]");
			while (true) {
				try {
					doSchedule();
				} catch (Exception ex) {
					_LOGGER.error("[RDP签名消费者未知异常:" + Thread.currentThread().getName() + "]", ex);
				}
		...
		private void doSchedule() {
			...
			try {
				...
				_LOGGER.info("[RDP签名数据:{}", messageStr);
				rdpNoticeSignConsumerService.saveMessage(messageStr);
				...
				if (ex.getMessage().trim().equals("Connection closed") || ex.getMessage().trim().startsWith("Connection reset")) {
					...
					_LOGGER.debug("[RDP签名消费者MNS连接异常,可忽略：{}，{}，{}]", ex.getErrorCode(), ex.getRequestId(), ex.getMessage());
				}
			} catch (Exception ex) {
				if (ex instanceof BaseException) {
					BaseException baseException = (BaseException) ex;
					if (baseException.getResponse() == ResponseEnum.FILE_SUMMARY_INCONSISTENCY) {
						_LOGGER.info("[RDP签名--文件摘要不匹配，删除通知:{}]", message.getMessageBodyAsRawString());
						queue.deleteMessage(message.getReceiptHandle());
					}
				} else {
					_LOGGER.error("[RDP签名消费者未知异常:" + Thread.currentThread().getName() + "]", ex);
